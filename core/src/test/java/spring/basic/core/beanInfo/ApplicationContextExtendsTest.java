...
public class ApplicationContextExtendsTest {
    ...
    @Test
    @DisplayName("부모 타입으로 조회 시, 자식 클래스가 둘 이상 있으면 중복 오류가 발생한다")
    void findBeanByParentTypeDuplication() {
//        DiscountPolicy parent = ac.getBean(DiscountPolicy.class);
        Assertions.assertThrows(NoUniqueBeanDefinitionException.class,
                () -> ac.getBean(DiscountPolicy.class));
    }

    @Test
    @DisplayName("부모 타입으로 조회 시, 자식 클래스가 둘 이상 있으면 빈 이름을 지정한다.")
    ...
    @Test
    @DisplayName("부모 타입으로 조회 시, 자식 클래스가 둘 이상 있으면 구현 타입을 지정한다.")
